...
export default function ProjectCreate() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await dialog.alert("建立專案時發生錯誤: " + error.message)
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="space-y-8 max-w-5xl mx-auto pb-12 animate-in fade-in duration-500">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="flex justify-end gap-4 pt-4">
           ...
           <Button type="submit" isLoading={loading} className="h-12 px-8 text-lg shadow-lg hover:shadow-xl transition-all hover:-translate-y-0.5 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white border-0">建立專案</Button>
        </div>
      </form>
    </div>
  )
